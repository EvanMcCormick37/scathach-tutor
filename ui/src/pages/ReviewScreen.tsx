...
export default function ReviewScreen() {
  ...
  useEffect(() => {
    api.getDueQuestions("untimed", "review").then((r) => {
  ...
  if (phase === "loading") return <Screen><div className="text-gray-600">Loading review queue…</div></Screen>;
  if (phase === "idle") return <Screen><EmptyQueue label="No questions due for review." /></Screen>;
  if (phase === "done") return <Screen><DoneCard count={queue.length} /></Screen>;
...
function ResultCard({ result, onNext }: { result: ReviewAnswerResult; onNext: () => void }) {
  return (
    <div className="space-y-4">
      ...
      <button onClick={onNext} className="w-full rounded-xl bg-brand-600 py-3 font-semibold text-white hover:bg-brand-500 transition">
        Next →
      </button>
    </div>
  );
...
function EmptyQueue({ label }: { label: string }) {
  return (
    <div className="text-center py-16 text-gray-600">
      ...
      <p>{label}</p>
      <Link to="/" className="mt-4 inline-block text-sm text-brand-400 hover:text-brand-300">Back to Dashboard</Link>
    </div>
  );
}
...
function DoneCard({ count }: { count: number }) {
  return (
    <div className="text-center py-16 space-y-4">
      ...
      <p className="text-gray-300">Review complete — {count} question{count !== 1 ? "s" : ""} done!</p>
